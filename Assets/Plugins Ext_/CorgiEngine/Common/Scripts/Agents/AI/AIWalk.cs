...
{	
	...
	public class AIWalk : MonoBehaviour
	{
	    ...
	    protected virtual void CheckForHoles()
	    {
			...
			RaycastHit2D raycast = MMDebug.RayCast(raycastOrigin, -transform.up, HoleDetectionRaycastLength, _controller.PlatformMask | _controller.MovingPlatformMask | _controller.OneWayPlatformMask | _controller.MovingOneWayPlatformMask,  Color.yellow, true);
	    ...
	    protected virtual void CheckForTarget()
	    {
			...
			RaycastHit2D raycast = MMDebug.RayCast(raycastOrigin,Vector2.left,ViewDistance,MoveOnSightLayer,Color.gray,true);
			// if we see a player
			if (raycast)
			{
				hit=true;
			...
			raycast = MMDebug.RayCast(raycastOrigin,Vector2.right,ViewDistance,MoveOnSightLayer,Color.gray,true);
			if (raycast)
			{
				hit=true;
			...
			else
			{
				...
				RaycastHit2D raycastObstacle = MMDebug.RayCast(raycastOrigin,_direction,ViewDistance,MoveOnSightObstaclesLayer,Color.blue,true);
