...
// -----------------------------------------------------------------------------
// Frame blending (Remade the way compression is done)
...
// Fragment the RGB to YCBCR convertion matrix in order to gain dot performances
CompressorOutput FragFrameCompress(VaryingsDefault i)
{
	...
	// Cutting the edges of subsampling to make it feel more "smooth"
    CompressorOutput o;
    o.luma = dot(kY, rgb_y);
...
half4 FragFrameBlending(VaryingsMultitex i) : SV_Target
{
	...
	// Rewrote the whol DecodeHistory with uvs to send textures sampling a lightweight emission
    #if UNITY_COLORSPACE_GAMMA
    half3 acc = src.rgb;
    #else
